...
while(valid_row == False or valid_column == False):
    ...
    if (player_ship_row in avail_rows):
        ...
        print("Invalid row.")
        valid_row = False
        # This avoids situations where the row
        # was good before but is now bad
        # a7
        # q3
    try:
...
while(valid_row == False or valid_column == False):
    ...
    try:
        ...
        print("Please enter a number for the column.")
    if (missile_col >= 0 and missile_col < len(player_missile_grid["a"])):
        valid_column = True
    else:
        print("Invalid column.")
        valid_column = False
...
# get full code from online, struggled to keep up. too tired. fuck calc tests